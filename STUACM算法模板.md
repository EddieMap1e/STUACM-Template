...
> 假如房屋是首尾相接的话,那么可以分成1 ~ n-1和2 ~ n两段来进行动态规划
...
vector<int> head(n+5,-1);	//存节点i的第一条边的编号 a-->b  存的是a的编号
...
> **朴素Dijkstra**
>
> 有向图中求一个点与其他点的最短距离,即单源最短路,且边权重只能为正
>
...
vector<int> visited(n2,false);	//表示是否在一次匹配中访问过节点 防止重边重复访问
